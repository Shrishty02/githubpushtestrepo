...
select count(*) from bank_details #no. of rows
select sum(balance) from bank_details
select avg(balance) from bank_details
select min(balance) from bank_details #mainimum balance
...
DELIMITER &&
create procedure select_pre_filter4(IN var int, IN var1 varchar(30))
select  * from bank_details
BEGIN
	select  * from bank_details
...
call select_pre_filter4(200, 'service')
...
select bank_details.age , bank_detailsjob, marital from bank_details as b inner join bank_details1 b1 on b.age =b1.age on age 
...
#delete database Drop database ineuron